...
if rank == 0:
    ...
    print(f"Master with rank {rank} is asleep at {cur_time(time.time())}")
    sleep_for()
    print(f"Master with rank {rank} is awake at {cur_time(time.time())}\n")
    message = MPI.COMM_WORLD.recv(source=1, tag=0)
    print(f"Master with rank {rank} received message:")
    print(f"'{message}' which translates to '{cur_time(message)}'")
    print(f"from {(time.time() - message):.2f} seconds ago")
